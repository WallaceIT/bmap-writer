...
int BmapWriteImage(const std::string &imageFile, const bmap_t &bmap, const std::string &device,
    ...
    try {
        size_t decHead = 0;
        int rwMode = (checksumMode == CHECKSUM_WRITTEN) ? O_RDWR : O_WRONLY;

        dev_fd = open(device.c_str(), rwMode | O_CREAT | O_SYNC, S_IRUSR | S_IWUSR);
        if (dev_fd < 0) {
        ...
        for (const auto &range : bmap.ranges) {
            ...
            if (checksumMode != NO_CHECKSUM) {
                checksum.finish();
                if (checksum.getChecksum() != range.checksum) {
                    std::stringstream err;
                    err << "Checksum verification failed for range: " << range.startBlock << "-" << range.endBlock << std::endl;
                    err << "Computed Checksum: " << checksum.getChecksum() << std::endl;
